...
app.get('/problems', async (req, res) => {
  try {
    ...
    let allowedManageProblem = await res.locals.user.hasPrivilege('manage_problem');
    let query = Problem.createQueryBuilder();
    if (!res.locals.user || !allowedManageProblem) {
      if (res.locals.user) {
        query.where(new TypeORM.Brackets(qb => {
          qb.where('is_public = 1')
            .orWhere('user_id = :user_id', {user_id: res.locals.user.id});
        })).andWhere('allow_level <= :allow_level', {allow_level: res.locals.user.level});
      } else {
    ...
    res.render('problems', {
      ...
      allowedManageProblem: res.locals.user && allowedManageProblem,
      problems: problems,
      paginate: paginate,
      curSort: sort,
      curOrder: order === 'asc'
...
app.get('/problems/search', async (req, res) => {
  try {
    ...
    if (!res.locals.user || !allowedManageProblem) {
      if (res.locals.user) {
        query.where(new TypeORM.Brackets(qb => {
          ...
          .andWhere('allow_level <= :level', {level: res.locals.user.level});
      } else {
        query.where('is_public = 1')
          .andWhere(new TypeORM.Brackets(qb => {
            qb.where('title LIKE :title', {title: `%${req.query.keyword}%`})
    ...
    res.render('problems', {
      allowedManageTag: res.locals.user && await res.locals.user.hasPrivilege('manage_problem_tag'),
      allowedManageProblem: allowedManageProblem,
      problems: problems,
      paginate: paginate,
      curSort: sort,
      curOrder: order === 'asc'
...
app.post('/problem/:id/manage', app.multer.fields([{name: 'testdata', maxCount: 1}, {
